...
class Trainer(object):
  ...
  def __init__(self, flags):
    ...
    self.filename = os.path.join(dirname, 'tables/q_table_extended_state_vector.pickle')
    self.logFileName = os.path.join(dirname, 'logs/extended_state_logger.pickle')
    try:
        ...
        self.q_table = {}
    self.eval_iter = 20000
    self.eval_timer = 100000
  ...
  def encode_state(self, observation):
    ...
    cards = observation['pyhanabi'].card_knowledge()[0]
    if len(cards) != 2:
        state += ''.join(['0']*5)
    for cardObj in cards:
        ...
        if rank != None:
            ...
            continue
        for i in range(5):
            if cardObj.rank_plausible(i) != True:
                card_str[i] = '0'
        state += ''.join(card_str)
    ...
    cards = observation['pyhanabi'].card_knowledge()[1]
    if len(cards) != 2:
        state += ''.join(['0']*5)
    for cardObj in cards:
        rank = cardObj.rank()
    ...
    # the length of state string should be 35
    if len(state) != 35:
        print(observation)
        print(len(state))
        ...
        while not done:
            ...
            for agent_id, agent in enumerate(agents):
                ...
                if observations['player_observations'][agent_id]['current_player_offset'] == 0:
                    ...
                    observation = observations['player_observations'][agent_id]
                    print('Observation:')
                    print(observation['pyhanabi'].card_knowledge()[0][0])
                    print("***********")
  ...
  def explore_phase(self, num_iters = 2000000):
    ...
    for _ in tqdm(range(num_iters)):
        ...
        if _ % self.eval_timer == 0:
            log.append(self.evaluate(self.eval_iter))
  ...
  def train(self):
    ...
    for episode in tqdm(range(num_episodes)):
        ...
        if episode%self.eval_timer == 0 and episode != 0:
            
            print('Saving table, log')
            log.append(self.evaluate(self.eval_iter))
            try:
                with open(self.filename, 'wb') as fp:
                    pickle.dump(self.q_table, fp, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    log_dict = {'log': log, 'freq':self.eval_timer, 'iters': self.eval_iter}
    try:
    ...
    x = np.linspace(0,len(log), len(log))*self.eval_timer
    plt.plot(x, log)
    plt.scatter(x,log)
    plt.xlabel('Episode')
    plt.ylabel('Predicted average score')
    plt.title('Extended state q-learning in simple Hanabi')
    plt.savefig('extended_state_q_learning.png')
